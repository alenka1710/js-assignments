...
function angleBetweenClockHands(date) {
	...
	var angleHR, angleMN, angle;
	if (hours>11){ hours=hours-12;}
	angleHR=((0.5*60*hours)+(0.5*minutes))*Math.PI/180;
	angleMN=(6*minutes)*Math.PI/180;
	angle=(Math.abs(angleHR-angleMN));
	return Math.min(angle,Math.abs(2*Math.PI-angle));
}
