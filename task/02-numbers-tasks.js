...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    ...
    denominator=(Math.sqrt(Math.pow(x1,2)+Math.pow(y1,2)))*(Math.sqrt(Math.pow(x2,2)+Math.pow(y2,2)));
...
function getParallelipidedDiagonal(a,b,c) {
    var diag2;
    var otvet;
    diag2=Math.pow(a,2)+Math.pow(b,2);
    otvet=Math.sqrt(Math.pow(c,2)+diag2);
    return otvet;
}
...
function roundToPowerOfTen(num, pow) {
  ...
  switch(pow){
    case 0:
      number=Math.round(num/Math.pow(10,pow))*Math.pow(10,pow)
     break;
    case 1:
      number=Math.round(num/Math.pow(10,pow))*Math.pow(10,pow);
      break; 
